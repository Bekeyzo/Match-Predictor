package models

type PredictionRequest struct {
	HomeTeam   string `json:"home_team"`
	AwayTeam   string `json:"away_team"`
	MatchDate  string `json:"match_date"`
	LeagueCode string `json:"league_code"`
}

type PredictionResult struct {
	Fixture                string  `json:"fixture"`
	HomeTeam               string  `json:"home_team"`
	AwayTeam               string  `json:"away_team"`
	PredictedResult        string  `json:"predicted_result"`
	HomeWinProbPct         float64 `json:"home_win_prob_pct"`
	DrawProbPct            float64 `json:"draw_prob_pct"`
	AwayWinProbPct         float64 `json:"away_win_prob_pct"`
	ExpectedHomeGoals      float64 `json:"expected_home_goals"`
	ExpectedAwayGoals      float64 `json:"expected_away_goals"`
	MostLikelyScore        string  `json:"most_likely_score"`
	ProbMostLikelyScorePct float64 `json:"prob_most_likely_score_pct"`
	BttprobPct             float64 `json:"btts_prob_pct"`
	ExpectedTotalGoals     float64 `json:"expected_total_goals"`
	ProbOver25Pct          float64 `json:"prob_over_2_5_pct"`
	ProbOver15Pct          float64 `json:"prob_over_1_5_pct"`
	ProbOver35Pct          float64 `json:"prob_over_3_5_pct"`
	ExpectedHomeCorners    float64 `json:"expected_home_corners"`
	ExpectedAwayCorners    float64 `json:"expected_away_corners"`
	ExpectedTotalCorners   float64 `json:"expected_total_corners"`
	ExpectedHomeCards      float64 `json:"expected_home_cards"`
	ExpectedAwayCards      float64 `json:"expected_away_cards"`
	ExpectedTotalCards     float64 `json:"expected_total_cards"`
	ExpectedHomeShots      float64 `json:"expected_home_shots"`
	ExpectedAwayShots      float64 `json:"expected_away_shots"`
	ExpectedTotalShots     float64 `json:"expected_total_shots"`
	ExpectedHomeFouls      float64 `json:"expected_home_fouls"`
	ExpectedAwayFouls      float64 `json:"expected_away_fouls"`
	ExpectedTotalFouls     float64 `json:"expected_total_fouls"`
	ProbOver205Shots       float64 `json:"prob_over_20_5_shots"`
	ProbOver235Shots       float64 `json:"prob_over_23_5_shots"`
	ProbOver265Shots       float64 `json:"prob_over_26_5_shots"`
	ProbOver75Sot          float64 `json:"prob_over_7_5_sot"`
	ProbOver95Sot          float64 `json:"prob_over_9_5_sot"`
	ExpectedBlendedShots   float64 `json:"expected_blended_shots"`
	ExpectedHomeSaves      float64 `json:"expected_home_saves"`
	ExpectedAwaySaves      float64 `json:"expected_away_saves"`
	ExpectedTotalSaves     float64 `json:"expected_total_saves"`
	ProbOver45Saves        float64 `json:"prob_over_4_5_saves"`
	ProbOver55Saves        float64 `json:"prob_over_5_5_saves"`
	ProbOver65Saves        float64 `json:"prob_over_6_5_saves"`
	ProbHomeOver185Shots   float64 `json:"prob_home_over_18_5_shots"`
	ProbAwayOver185Shots   float64 `json:"prob_away_over_18_5_shots"`
	ProbOver85Corners      float64 `json:"prob_over_8_5_corners"`
	ProbOver105Corners     float64 `json:"prob_over_10_5_corners"`
	ProbOver245Fouls       float64 `json:"prob_over_24_5_fouls"`
	ProbHomeOver125Fouls   float64 `json:"prob_home_over_12_5_fouls"`
	ProbAwayOver125Fouls   float64 `json:"prob_away_over_12_5_fouls"`
	ProbHomeOver45Sot      float64 `json:"prob_home_over_4_5_sot"`
	ProbAwayOver45Sot      float64 `json:"prob_away_over_4_5_sot"`
	ProbHomeKeeperOver35Saves float64 `json:"prob_home_keeper_over_3_5_saves"`
	ProbAwayKeeperOver35Saves float64 `json:"prob_away_keeper_over_3_5_saves"`
	ExpectedHomeSot        float64 `json:"expected_home_sot"`
	ExpectedAwaySot        float64 `json:"expected_away_sot"`
	ExpectedTotalSot       float64 `json:"expected_total_sot"`
	InsufficientData       *bool   `json:"insufficient_data,omitempty"`
	StaleData              *bool   `json:"stale_data,omitempty"`
	Reason                 *string `json:"reason,omitempty"`
}
